...
const sidebar = new Sidebar({
  ...
  onNewChat:         ()  => { currentConversationId = null; chat.clearMessages(); },
...
async function handleSend(text) {
  ...
  try {
    ...
    if (intent.type === 'prayer') {
      ...
      setTimeout(() => {
        player.open({ ...prayer, audioUrl: null }); // audioUrl comes from API in production
      }, 400);
    } else {
      // Full AI response from backend
...
async function loadConversation(idOrText) {
  ...
  try {
    ...
    for (const msg of data.messages) {
      if (msg.role === 'user') chat.appendUserMessage(msg.content);
      else chat.appendBotMessage({ text: msg.content, tradition: msg.tradition, cite: msg.citation });
    }
