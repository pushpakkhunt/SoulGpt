...
class ApiClient {
  ...
  /** Read body once; support JSON or HTML/plain proxy errors. Always include status code. */
  async _throwIfNotOk(res) {
    ...
    if (raw) {
      try {
        ...
        msg = this._errorMessage(body, status);
      } catch {
        const t = raw.trim().replace(/\s+/g, ' ');
        msg = (t.slice(0, 400) || msg);
      }
    }
    if (!String(msg).includes(`[${status}]`)) {
      msg = `${msg} [${status}]`;
    }
    throw new Error(msg);
  }
   ...
   * @param {string} [convId]  - Conversation ID (null = new conversation)
   */
  async sendMessage(message, tradition = 'all', language = 'en', convId = null) {
    return this._post('/chat/message', { message, tradition, language, conversationId: convId });
  }
  ...
  async getPrayerStream(prayerKey) {
    // Returns a signed URL for the audio stream
